...
os.system('wget http://www.caloriemama.ai/img/examples/Image5.jpeg')

#size image to 544 x 544 px (API req)
#wget
img = Image.open("Image5.jpeg")
...
for i, blob in enumerate(jsonheader):
    foodnutrition = blob['nutrition']
    foodnutrition['portion'] = blob['servingSizes'][0]['unit']
    foodnutrition['score'] = blob['score']
    foodnames.append(blob['name']) #collect names in case duplicated
